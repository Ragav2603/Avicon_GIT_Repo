...
def generate_recommendations(metrics: List[Dict], score: float) -> List[str]:
    ...
    if score < 40:
        recommendations.append(
            "Critical: Schedule immediate training sessions to improve adoption."
        )

    for metric in metrics:
        if metric.get("trend") == "down":
            recommendations.append(
                f"⚠️ {metric['metric_name']} is declining. Consider user feedback sessions."
            )
        if metric.get("current_value", 0) < 30:
            recommendations.append(
                f"📉 {metric['metric_name']} is low. Review implementation and user documentation."
            )

    if score >= 70:
        recommendations.append(
            "✅ Strong adoption. Consider expanding to additional user groups."
        )
...
async def create_telemetry_connection(connection: TelemetryConnectionCreate):
    ...
    if not submission:
        raise HTTPException(
            status_code=404, detail="Submission not found or not accepted"
        )
    if submission.get("status") != "accepted":
        raise HTTPException(
            status_code=400,
            detail="Only accepted submissions can have telemetry connections",
        )
    ...
    conn_data = {
        ...
        "status": "pending"
        if connection.provider != TelemetryProvider.MANUAL
        else "connected",
        "last_sync": None,
        "created_at": now,
    }
...
async def list_telemetry_connections(
    submission_id: Optional[str] = None, rfp_id: Optional[str] = None
):
...
async def sync_telemetry_data(connection_id: str, background_tasks: BackgroundTasks):
    ...
    if conn["provider"] == TelemetryProvider.MANUAL:
        raise HTTPException(
            status_code=400, detail="Manual connections do not support sync"
        )
...
async def simulate_telemetry_sync(connection_id: str):
    ...
    for metric_type in metric_types:
        ...
        for i in range(30):
            ...
            data_points.append(
                {"timestamp": date.isoformat(), "value": round(value, 1)}
            )
...
async def delete_telemetry_connection(connection_id: str):
    ...
    metrics_to_delete = [
        k for k, v in adoption_metrics.items() if v["connection_id"] == connection_id
    ]
...
async def create_manual_metric_entry(entry: ManualMetricEntry):
    ...
    adoption_metrics[metric_id] = {
        ...
        "trend": "stable",
        "data_points": [
            {"timestamp": entry.period_end.isoformat(), "value": entry.value}
        ],
...
async def get_adoption_summary(submission_id: str):
    ...
    metrics = [
        m for m in adoption_metrics.values() if m["submission_id"] == submission_id
    ]
...
async def list_accepted_submissions(
    airline_id: Optional[str] = None, vendor_id: Optional[str] = None
):
    ...
    for sub in submissions:
        metrics = [
            m for m in adoption_metrics.values() if m["submission_id"] == sub["id"]
        ]
        sub["adoption_score"] = calculate_adoption_score(metrics)
        sub["health_status"] = determine_health_status(sub["adoption_score"])
        sub["has_telemetry"] = any(
            c["submission_id"] == sub["id"] for c in telemetry_connections.values()
        )
