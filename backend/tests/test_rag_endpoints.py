...
SUPABASE_URL = os.environ.get(
    "SUPABASE_URL", "https://aavlayzfaafuwquhhbcx.supabase.co"
)
SUPABASE_ANON_KEY = os.environ.get(
    "SUPABASE_ANON_KEY",
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFhdmxheXpmYWFmdXdxdWhoYmN4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njg2NDMyNTcsImV4cCI6MjA4NDIxOTI1N30.gst2u0jgQmlewK8FaQFNlVI_q4_CvFJTYytuiLbR55k",
)
...
class TestAuthProtection:
    ...
    def test_drafts_create_requires_auth(self, api_client):
        ...
        response = api_client.post(
            f"{BASE_URL}/api/drafts", json={"title": "Test", "content": "test"}
        )
        assert response.status_code == 401
...
class TestRAGEndpoints:
    ...
    def test_rfp_draft_generation_with_template(self, authenticated_client):
        ...
        assert (
            "IFE" in draft_text
            or "Entertainment" in draft_text
            or "Flight" in draft_text
        )
        ...
        print(
            f"RFP Draft generated: {len(draft_text)} characters in {data['latency_ms']}ms"
        )
