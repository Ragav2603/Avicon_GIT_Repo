...
class TestTeamTemplatesEndpoints:
    ...
    def test_list_team_templates_with_category_filter(self):
        ...
        assert response.status_code == 401, f"Expected 401, got {response.status_code}"
        print(
            "✅ GET /api/team-templates with category filter returns 401 without auth"
        )
    ...
    def test_update_team_template_requires_auth(self):
        ...
        response = requests.put(
            f"{BASE_URL}/api/team-templates/some-template-id", json=payload
        )
        assert response.status_code == 401, f"Expected 401, got {response.status_code}"
        data = response.json()
        assert "detail" in data, "Expected error detail in response"
...
class TestTeamTemplatesWithInvalidToken:
    ...
    def test_create_with_invalid_token(self):
        headers = {"Authorization": "Bearer invalid_token_123"}
        payload = {"title": "Test", "content": "Test content", "category": "General"}
        response = requests.post(
            f"{BASE_URL}/api/team-templates", json=payload, headers=headers
        )
        assert response.status_code == 401, (
            f"Expected 401 with invalid token, got {response.status_code}"
        )
    ...
    def test_use_template_with_invalid_token(self):
        headers = {"Authorization": "Bearer invalid_token_123"}
        response = requests.post(
            f"{BASE_URL}/api/team-templates/some-id/use", headers=headers
        )
        assert response.status_code == 401, (
            f"Expected 401 with invalid token, got {response.status_code}"
        )
